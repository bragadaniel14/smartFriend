...
class scene4BadViewController: UIViewController {
    ...
    @objc private func moveStory(button: UIButton) {
        ...
        if(index == 3){
            performSegue(withIdentifier: "toMakeOver", sender: self)
        }
    }
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        nextt.addTarget(self, action: #selector(scene4BadViewController.moveStory(button:)), for: UIControlEvents.touchUpInside)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        guard let scene = segue.destination as? makeOverViewController else{
            fatalError("Deu ruim")
        }
        scene.player = player
