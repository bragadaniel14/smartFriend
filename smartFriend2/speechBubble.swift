...
class SpeechBubble: UIView {
    ...
    private func resize(label: UILabel){
        let newSize = label.sizeThatFits(CGSize(width: 200, height: 60))
        label.frame.size = newSize
        // label.lineBreakMode = NSLineBreakMode.ByWordWrapping
    }
    
    required convenience init(baseView: UIView, text: String, fontSize: CGFloat = 20) {
        ...
        let width = newSize.width + padding * 3 // 50% more padding on width
        let height = newSize.height + triangleHeight + padding * 2
        ...
        label.frame = CGRect(x: padding, y: padding, width: newSize.width + padding, height: newSize.height)
    ...
    override func draw(_ rect: CGRect) {
        ...
        path.addArc(withCenter: CGPoint(x: bubble.minX, y: bubble.maxY), radius: radius2, startAngle: CGFloat(M_PI_2), endAngle: CGFloat(M_PI), clockwise: true)
        path.addArc(withCenter: CGPoint(x: bubble.minX, y: bubble.minY), radius: radius2, startAngle: CGFloat(M_PI), endAngle: CGFloat(-M_PI_2), clockwise: true)
        path.close()
